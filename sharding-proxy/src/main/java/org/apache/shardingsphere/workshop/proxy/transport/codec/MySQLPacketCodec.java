...
package org.apache.shardingsphere.workshop.proxy.transport.codec;
...
public final class MySQLPacketCodec extends ByteToMessageCodec<MySQLPacket> {
    ...
    private final MySQLPacketCodecEngine codecEngine = new MySQLPacketCodecEngine();
    
    @Override
    protected void decode(final ChannelHandlerContext context, final ByteBuf in, final List<Object> out) {
        int readableBytes = in.readableBytes();
        if (!codecEngine.isValidHeader(readableBytes)) {
            return;
        }
        if (log.isDebugEnabled()) {
            log.debug("Read from client {} : \n {}", context.channel().id().asShortText(), ByteBufUtil.prettyHexDump(in));
        }
        codecEngine.decode(context, in, out, readableBytes);
    }
